...
define void @_Z6serialiffPf(i32 %N, float %m, float %m2, float* %A) #0 {
entry:
...
for.body:                                         ; preds = %for.cond
  %2 = load float, float* %m.addr, align 4
...
if.then:                                          ; preds = %for.body
  %6 = load float, float* %m.addr, align 4
  store float %6, float* %m2.addr, align 4
  ...
  store float %9, float* %m.addr, align 4
  br label %if.end10
...
if.else:                                          ; preds = %for.body
  ...
  %13 = load float, float* %m2.addr, align 4
...
if.then7:                                         ; preds = %if.else
  ...
  store float %16, float* %m2.addr, align 4
  br label %if.end
